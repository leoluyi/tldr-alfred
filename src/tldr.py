...
def find_page_location(command):
    ...
    with io.open(
        os.path.join(alfred_workflow_data, "index.json"), encoding="utf-8"
    ) as f:
        index = json.load(f)
    command_list = [item["name"] for item in index["commands"]]
    if command not in command_list:
        return os.path.join(
            os.path.join(repo_directory, "pages"),
    ...
    page_path = os.path.join(
        os.path.join(repo_directory, "pages"), os.path.join(platform, command + ".md")
    )
    return page_path
...
def clone():
    ...
    if not os.path.exists(repo_directory):
        child = subprocess.Popen(
            [
                "git clone https://github.com/tldr-pages/tldr.git " + '"' + str(repo_directory) + '"'
            ],
            shell=True,
...
def update(days=0):
    if days > 0 and os.path.exists(os.path.join(alfred_workflow_data, "config.json")):
        with io.open(
            os.path.join(alfred_workflow_data, "config.json"), encoding="utf-8"
        ) as f:
            try:
                config = json.load(f)
            except Exception:
        ...
        if (
            datetime.datetime.now().date() - datetime.datetime.strptime(config["update_date"], "%Y%m%d").date()
        ).days < days:
            return
    os.chdir(repo_directory)
    local = subprocess.check_output("git rev-parse HEAD".split()).strip()
...
def parse_args(query=""):
    query = query.split()
    dic = {"isUpdate": False, "platform": default_platform, "command": ""}
    try:
...
def hint(command, platform=""):
    ...
    for item in index["commands"]:
        if (
            platform in item["platform"] or "common" in item["platform"]
        ) and command == item["name"][0 : len(command)]:
            if platform == "osx":
